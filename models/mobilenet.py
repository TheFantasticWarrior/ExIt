# https://github.com/jaiwei98/MobileNetV4-pytorch/tree/main
import torch
...
class MobileNetV4(nn.Module):
    def __init__(self):
        ...
        """
        super().__init__()
        self.spec = MNV4ConvSmall_BLOCK_SPECS
        ...
        self.multihead_attn = nn.MultiheadAttention(embed_dim=96, num_heads=8)
    ...
    def forward(self, x,data):
        ...
        x3 = self.layer3(x2)
        B, C, H, W = x3.shape
        image_features_flat = x3.view(B, C, -1).permute(2, 0, 1)  # Shape: (H*W, B, C)

        # Project text embeddings and expand
        data = data.unsqueeze(0).repeat(H*W,1,1)  # Shape: (1, B, C)

        # Apply multihead attention
        attn_output, _ = self.multihead_attn(data, image_features_flat, image_features_flat)
        attn_output = attn_output.permute(1, 2, 0).view(B, C, H, W)
        x3=attn_output
        x4 = self.layer4(x3)
        x5 = self.layer5(x4)
        x5 = nn.functional.adaptive_avg_pool2d(x5, 1 )
        out=F.softmax(self.fc(x5.squeeze()),-1)
        return out
